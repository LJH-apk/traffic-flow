...
OUTPUT_DIR   = Path("/Users/liujiahang/科研/交通流算法")
DEVICE       = "mps"
SAMPLE_EVERY = 25        # 每隔N帧采样一帧（约1秒一帧）
MAX_SAMPLES  = 200       # 最多采样帧数
CONF_GT      = 0.25      # yolo26x 置信度阈值（作为伪GT）
CONF_PRED    = 0.25      # yolo26m 置信度阈值

TRAFFIC_CLASSES = {2: "car", 5: "bus", 7: "truck", 3: "motorcycle", 1: "bicycle", 0: "person"}
IOU_THRESHOLDS  = np.linspace(0.5, 0.95, 10)  # COCO标准：0.5到0.95步长0.05
...
def compute_ap_at_iou(gt_list, pred_list, iou_thresh):
    """
    gt_list  : list of np.array shape (N,4) per image
    pred_list: list of np.array shape (M,5) per image, cols=[x1,y1,x2,y2,conf]
    """
    all_confs   = []
    all_tp      = []
    n_gt_total  = 0
...
def main():
    print("加载模型...")
    model_gt   = YOLO("yolo26x.pt")   # 伪GT生成器
    model_pred = YOLO("yolo26l.pt")   # 待评测模型
    ...
    for i, frame in enumerate(frames):
        ...
        for cid in TRAFFIC_CLASSES:
            ...
            gt_cid = gt_boxes.xyxy[gt_boxes.cls.int() == cid].cpu().numpy() if len(gt_boxes) else np.zeros((0, 4))
    ...
    for cid, cname in TRAFFIC_CLASSES.items():
        ...
        ap5095 = np.mean([compute_ap_at_iou(class_gt[cid], class_pred[cid], t)
                          for t in IOU_THRESHOLDS])
        results[cname] = {"AP50": round(ap50, 4), "AP50_95": round(ap5095, 4)}

    map50   = np.mean([v["AP50"]    for v in results.values()])
    map5095 = np.mean([v["AP50_95"] for v in results.values()])

    # ── 打印结果 ──
    print(f"\n{'='*50}")
    print(f"  yolo26m vs yolo26x（伪GT）评测结果")
    print(f"  采样帧数: {len(frames)}  置信度阈值GT={CONF_GT} / Pred={CONF_PRED}")
    print(f"{'='*50}")
    ...
    # ── 保存 JSON ──
    output = {"model": "yolo26l", "pseudo_gt": "yolo26x",
              "samples": len(frames), "mAP50": round(map50, 4),
              "mAP50_95": round(map5095, 4), "per_class": results}
...
def _plot(results, map50, map5095):
    ...
    ax.set_title("yolo26m 逐类别 AP（伪GT by yolo26x）")
    ...
    bars   = ax2.bar(labels, vals, color=colors, width=0.4, zorder=3)
    ax2.set_ylim(0, 1.05); ax2.set_ylabel("mAP")
    ax2.set_title("yolo26m 整体 mAP")
    ...
    plt.suptitle(f"评测数据集：比赛测试视频（采样{sum(1 for _ in results)}类）", fontsize=11)
    plt.tight_layout()
