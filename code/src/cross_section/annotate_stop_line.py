...
用法：
  python3 src/cross_section/annotate_stop_line.py --entrance 东进口

操作与车道线标注器一致，只需要在 Lane1 上点停车线两个端点，然后按 S 保存。
输出: src/assets/calibrations/<进口>/stop_lines.json
"""
...
def main() -> None:
    ...
    parser.add_argument("--image", type=Path, help="参考图路径，默认 src/assets/calibrations/<进口>/ref.jpg")
    parser.add_argument("--out", type=Path, help="输出 JSON，默认 src/assets/calibrations/<进口>/stop_lines.json")
